...
def split_into_scenes(text):
    ...
    cleaned_scenes = [re.sub(r'# # \* \(CONTINUED\)Red \d+\.\d+\.\d+ \d+\.', '', scene) for scene in scenes]  # Clean up scenes
    return cleaned_scenes

def split_into_scenes(text):
    # Add spaces before scene headers to help with splitting
    text = re.sub(r'\b(INT\.|EXT\.)', r' \1', text)

    # Remove unwanted lines
    text = re.sub(r'# #\* \(CONTINUED\)Red \d+\.\d+\.\d+ \d+\.', '', text)

    # Split the text into scenes based on the scene headers
    scenes = re.split(r' (?=INT\.|EXT\.)', text)
...
if uploaded_file is not None:
    ...
    for i, scene in enumerate(scenes):
        ...
        st.subheader(f"Scene {i+1}")  # Changed this line
        st.write(scene)  # Display the scene text
